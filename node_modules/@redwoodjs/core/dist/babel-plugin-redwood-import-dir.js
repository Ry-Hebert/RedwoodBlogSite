...
const generateTypes = modulePath => {
  // TODO:
  ...
  return `// @ts-expect-error\ndeclare module '${modulePath.replace('../', 'src/')}';`;
};
/**
  ...
  return {
    ...
    visitor: {
      ImportDeclaration(p, state) {
        ...
        const dirFiles = (0, _filter.default)(_context2 = _glob.default.sync(importGlob, {
          cwd
        })).call(_context2, n => !(0, _includes.default)(n).call(n, '.test.')); // ignore `*.test.*` files.
        ...
        if (options.host.writeFileSync) {
          options.host.writeFileSync(_path.default.join(options.generateTypesPath, `import-dir-${importName}.d.ts`), generateTypes(importGlob));
        }
      }
